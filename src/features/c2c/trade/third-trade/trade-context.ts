...
export function useTradeForm(ad: ITradeAdItem, defaultUnitIsAmount = false) {
  ...
  const maxAmount = formatNumberDecimal(ad.maxAmount, amountDigit, amountRoundStrategy)
  const minAmount = formatNumberDecimal(ad.minAmount, amountDigit, amountRoundStrategy)

  const minCount = formatNumberDecimal(
    decimalUtils.SafeCalcUtil.div(ad.minAmount, price),
    countDigit,
    countRoundStrategy,
    true
  )
  const maxCount = formatNumberDecimal(
    decimalUtils.SafeCalcUtil.div(ad.maxAmount, price),
    countDigit,
    countRoundStrategy,
    true
  )
  ...
  const adLimitError =
    ...
    ((unitIsAmount && (Number(amount) > Number(maxAmount) || Number(amount) < Number(minAmount))) ||
      (!unitIsAmount && (Number(count) > Number(maxCount) || Number(count) < Number(minCount))))
